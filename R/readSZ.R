...
readSZ <- function(path = getwd(),
    ...
    for(i in 1:length(file)){
        ...
        if(do.meta){
            ...
            metaTMP <- rbind(metaTMP,mData)
            if(i/100 == floor(i/100) | i==length(file)){meta <- rbind(meta,metaTMP)
                                                        metaTMP <- NULL}
        }
        if(do.text){
            ...
            for(j in to_delete){
                text_new <- gsub(pattern = j, replacement = " ", x = text_new, perl = TRUE)
            }
            text_new <- strsplit(text_new, "<P>|<P [^>]*>", perl = TRUE)
            text_new <- lapply(text_new, removeXML)
            names(text_new) <- id
            textTMP <- as.list(c(textTMP, text_new))
            if(i/100 == floor(i/100) | i==length(file)){text <- as.list(c(text, textTMP))
                                                        textTMP <- NULL}
        }
    }
