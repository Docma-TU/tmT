...
#'         (print(file[i]))
#'         openfile <- file(paste("Daten/", file[i], sep = ""),
#'             open = "rt")
...
#'             lines)
#'         if (en[i]) {
#'             artikel <- strsplit(paste(lines, collapse = " "),
#'                 "</Document>")[[1]]
#'         }
#'         else {
#'             artikel <- strsplit(paste(lines, collapse = " "),
...
#'                 quelle <- sapply(quelle, function(x) {
#'                   x[2]
#'                 })
...
#'                 land <- sapply(land, function(x) {
#'                   x[-1]
#'                 })
#'                 land <- sapply(land, function(x) unlist(strsplit(x,
#'                   "</Country>")))
#'                 land <- sapply(land, function(x) {
#'                   x[-seq(0, length(x), by = 2)]
#'                 })
...
#'                   x[-1]
#'                 })
#'                 industrie <- sapply(industrie, function(x) unlist(strsplit(x,
#'                   "</Industry>")))
#'                 industrie <- sapply(industrie, function(x) {
#'                   x[-seq(0, length(x), by = 2)]
#'                 })
...
#'                 autor <- sapply(autor, function(x) {
#'                   x[-1]
#'                 })
#'                 autor <- sapply(autor, function(x) unlist(strsplit(x,
#'                   " </Author>")))
#'                 autor <- sapply(autor, function(x) {
#'                   x[-seq(0, length(x), by = 2)]
#'                 })
...
#'                   x[-1]
#'                 })
#'                 rubrik <- sapply(rubrik, function(x) unlist(strsplit(x,
#'                   " </Category>")))
#'                 rubrik <- sapply(rubrik, function(x) {
#'                   x[-seq(0, length(x), by = 2)]
#'                 })
...
#'                 quelle <- as.numeric(sapply(quelle, function(x) {
#'                   x[1]
#'                 }))
...
#'                 x[-1]
#'             })
#'             klassifikation <- sapply(klassifikation, function(x) unlist(strsplit(x,
#'                 " </Klassifikation>")))
#'             klassifikation <- sapply(klassifikation, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'             thema <- sapply(thema, function(x) {
#'                 x[-1]
#'             })
#'             thema <- sapply(thema, function(x) unlist(strsplit(x,
#'                 " </Thema>")))
#'             thema <- sapply(thema, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'                 x[-1]
#'             })
#'             sachgruppe <- sapply(sachgruppe, function(x) unlist(strsplit(x,
#'                 " </Sachgruppe>")))
#'             sachgruppe <- sapply(sachgruppe, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'             serie <- sapply(serie, function(x) {
#'                 x[-1]
#'             })
#'             serie <- sapply(serie, function(x) unlist(strsplit(x,
#'                 " </Serie>")))
#'             serie <- sapply(serie, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'             })
#'             names(person) <- id
#'             mData <- data.frame(id, quelle, datum, titel, abstract,
#'                 UB, dachzeile, stringsAsFactors = FALSE)
...
#'             metamult$klassifikation <- c(metamult$klassifikation,
#'                 klassifikation)
