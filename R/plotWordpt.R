...
#' \code{ldaresult}
#' @param select List of integer vectors. Every list element is an 'or'
...
#' so that the argument \code{link} has no effect. Each integer vector
...
#' @param wordlist List of character vectors: Which words - linked by an "or" every time - 
...
#' @param xlab Ccharacter: Graphical parameter
...
plotWordpt <- function(object, docs, ldaresult, ldaID,
  ...
  if(rel){
    ...
    if(missing(main))
      main <- paste("Proportion of Words per Topics over Time - link:", link[1])
  }
  else{
    tab <- cbind(as.Date(rownames(splt2)), data.frame(splt2))
    if(missing(main))
      main <- paste("Counts of Words per Topics over Time - link:", link[1])
  }
