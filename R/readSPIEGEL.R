...
#'     for (i in year) {
#'         (print(i))
#'         openfile <- file(paste("Spiegel-Daten/", i, ".xml", sep = ""),
#'             open = "rt")
...
#'             jahr <- sapply(jahr, function(x) {
#'                 x[2]
#'             })
#'             jahr <- strsplit(jahr, "</jahrgang>")
#'             jahr <- as.numeric(sapply(jahr, function(x) {
#'                 x[1]
#'             }))
#'             nummer <- strsplit(artikel, "<nummer>")
#'             nummer <- sapply(nummer, function(x) {
#'                 x[2]
#'             })
#'             nummer <- strsplit(nummer, "</nummer>")
#'             nummer <- as.numeric(sapply(nummer, function(x) {
#'                 x[1]
#'             }))
...
#'             seiteS <- sapply(seiteS, function(x) {
#'                 x[2]
#'             })
#'             seiteS <- strsplit(seiteS, "</seite-start>")
#'             seiteS <- as.numeric(sapply(seiteS, function(x) {
#'                 x[1]
#'             }))
#'             seiteE <- strsplit(artikel, "<seite-ende>")
#'             seiteE <- sapply(seiteE, function(x) {
#'                 x[2]
#'             })
#'             seiteE <- strsplit(seiteE, "</seite-ende>")
#'             seiteE <- as.numeric(sapply(seiteE, function(x) {
#'                 x[1]
#'             }))
#'             seitentitel <- strsplit(artikel, "<seitentitel>")
#'             seitentitel <- sapply(seitentitel, function(x) {
#'                 x[2]
#'             })
...
#'             kurztitel <- sapply(kurztitel, function(x) {
#'                 x[2]
#'             })
...
#'             ressort <- sapply(ressort, function(x) {
#'                 x[2]
#'             })
...
#'             dokumentmerkmal <- sapply(dokumentmerkmal, function(x) {
#'                 x[2]
#'             })
...
#'             vorspann <- sapply(vorspann, function(x) {
#'                 x[2]
#'             })
...
#'             signatur <- sapply(signatur, function(x) {
#'                 x[-1]
#'             })
#'             signatur <- sapply(signatur, function(x) unlist(strsplit(x,
#'                 "</signatur>")))
#'             signatur <- sapply(signatur, function(x) {
...
#'             person <- sapply(person, function(x) {
#'                 x[-1]
#'             })
#'             person <- sapply(person, function(x) unlist(strsplit(x,
#'                 "</person>")))
#'             person <- sapply(person, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'             koerperschaft <- sapply(koerperschaft, function(x) {
#'                 x[-1]
#'             })
#'             koerperschaft <- sapply(koerperschaft, function(x) unlist(strsplit(x,
#'                 "</koerperschaft>")))
#'             koerperschaft <- sapply(koerperschaft, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
...
#'             firma <- sapply(firma, function(x) {
#'                 x[-1]
#'             })
#'             firma <- sapply(firma, function(x) unlist(strsplit(x,
#'                 "</firma>")))
#'             firma <- sapply(firma, function(x) {
#'                 x[-seq(0, length(x), by = 2)]
#'             })
#'             names(firma) <- id
#'             mData <- data.frame(id, jahr, nummer, datum, seiteS,
#'                 seiteE, seitentitel, kurztitel, rubrik, ressort,
#'                 dokumentmerkmal, dachzeile, titel, vorspann,
#'                 stringsAsFactors = FALSE)
#'             meta <- rbind(meta, mData)
#'             metamult$signatur <- c(metamult$signatur, signatur)
#'             metamult$person <- c(metamult$person, person)
#'             metamult$koerperschaft <- c(metamult$koerperschaft,
#'                 koerperschaft)
...
#'                 cData[[j]] <- sapply(absatz2, function(x) {
#'                   x[1]
#'                 })
#'             }
#'             names(cData) <- id
