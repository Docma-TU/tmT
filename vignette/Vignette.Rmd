...
## Visualisation of Words in Topic over Time - \texttt{plotTopicWord}
...
all(round(rowSums(tab[, -1]), 10) == 1)
```
This is confirmed by the call above. For some analysis maybe it could be interesting to take the other possible baseline, the topic counts, into account. Therefore an additional function is planned called \texttt{plotWordTopic}.
```{r, include = FALSE}
#Ist evtl auch interessant: \texttt{plotWordTopic} fuer geg. Topic den Verlauf der Anzahlen/Prop gewaehlter Worte (deafault: die 6 top.topic.words) innerhalb des Topics
```
