...
```
In the graphic above you could see, that the topic \textit{medicine} is nearly time independent. However the topic \textit{news} has a peak on the first half of the year 2012, the topic \textit{syndrome} on the first quarter of 2014. The topic \textit{data} is less represented. Therefore the peak at the end of 2013 is considerable. The light colors display the exact curves. Obvoiusly they alternate irrregular. 
...
Because this seems to be interesting topics \textit{T1.syndrome} (red curve), \textit{T3.health} (green) and \textit{T7.medicine} (blue) are plotted in a sediment plot. The chosen \texttt{unit} is \texttt{"bimonth"} (default is \texttt{"quarter"}).
```{r, eval = FALSE}
