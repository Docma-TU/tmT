...
out1 <- docLDA(corpus=c(D="",text), vocab=sort(unique(unlist(text))), ldacorrect=TRUE, excludeNA=TRUE)
out2 <- docLDA(corpus=c(D="",text), vocab=sort(unique(unlist(text))), ldacorrect=FALSE, excludeNA=TRUE)
out3 <- docLDA(corpus=c(D="",text), vocab=sort(unique(unlist(text))), ldacorrect=FALSE, excludeNA=FALSE)
...
expect_equal(docLDA(corpus=c(D="",text), vocab=sort(unique(unlist(text))), ldacorrect=TRUE, excludeNA=TRUE),list(A=A, B=B2,C=C2))
