...
lda1 <- LDAgen(documents=LDAdoc, K = 3L, vocab=wordlist$words, num.iterations = 20L, burnin = 70L, seed=124601, folder=paste0(getwd(),"/test"), num.words = 10L, LDA = TRUE)

text2 <- cleanTexts(text = text[c("B", "C", "F", "H")])
wordlist <- makeWordlist(text2)
LDAdoc <- LDAprep(text2, wordlist$words)
lda2 <- LDAgen(documents=LDAdoc, K = 3L, vocab=wordlist$words, num.iterations = 20L, burnin = 70L, seed=124601, folder=paste0(getwd(),"/test"), num.words = 10L, LDA = TRUE)
