...
```
Maybe you want to exclude very sparse texts from your analysis. Therefore you can use the function \texttt{filterCount}. The function regards words which only consist of letters and which are seperated by any word seperating symbol like whitespace or punctuation. Tokenized texts can also be processed. The function call \texttt{filterCount(corpus, count = 5)} for example deletes all texts from \texttt{corpus} which consist of four or less words. 
```{r}
mean(textCounts != filterCount(corpusDate, out = "count"))
```
The counts returned by \texttt{filterCount} need not to match the lengths of the tokenized texts exactly, because based on the way of preprocessing the corpus may contain symbols - which leads to smaller counts - or multiple words in one token. These multiple words could be unaffected by tokenization in \texttt{cleanTexts} if they are seperated by remaining symbols instead of whitespace and lead to higher counts.
