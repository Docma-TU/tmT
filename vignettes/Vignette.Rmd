...
plotFreq(corpusClear, wordlist = list(c("crisis", "war", "conflict")), link = "or",
...
```
\colorbox{yellow}{(Beispiel austauschen!)}As an illustration the first set is shown. The word \textit{will} does not fit into the set with the words \textit{bill}, \textit{tax}, \textit{billion} and \textit{budget}. Therefore the user would type \texttt{5} and press enter. If the user wants to mark nonsense topics he would type an \texttt{x} (in the summary the number of meaningful topics is shown) and \texttt{0} if he thinks there is no intruder word. Actually \textit{will} is the true intruder in the set above. As an example user input \texttt{c(5, 0, 0, 5, 1, 2, 0, 5, 3, 5)} is considered.
```{r}
print(intWords)
```
...
names(clustRes)
```
The same plot as above can be recreated by calling \texttt{plot(clustRes\$cluster)}. In the plot you can see the similarities concerning the hellinger distance of the topics. \colorbox{yellow}{(Inhaltlich checken!)}For example the plot hints to the similarity with regards to content between topic 4 and topic 9, which both contain economic terms with topic 9 focussing more on economic policy. Topic 5 and 8 both include words on international politics with topic 5 mentioning climate and trade and topic 8 concentrating on conflicts.   
...
plotTopic(object = corpusClear, ldaresult = result, ldaID = ldaID,
  select = c(1, 4, 8:9), rel = TRUE, curves = "both", smooth = 0.1, legend = "topleft")
```
...
Because this seems to be interesting topics \colorbox{yellow}{Inhaltlich checken!} \textit{T4.economy} (blue curve), \textit{T9.economic policy} (green) and \textit{T1.US politics} (red) are plotted in a sediment plot. The chosen \texttt{unit} is \texttt{"bimonth"} (default is \texttt{"quarter"}).
```{r, eval = FALSE}
plotArea(ldaresult = result, ldaID = ldaID, meta = corpusFiltered$meta,
  select = c(4, 9, 1), unit = "bimonth", sort = FALSE)
```
...
plotArea(ldaresult = result, ldaID = ldaID, meta = corpusFiltered$meta,
  select = c(4, 9, 1), unit = "bimonth", sort = FALSE)
```
\colorbox{yellow}{(Inhaltlich checken!)} Examplary interpretation: The topic \textit{T4.economy} increases over time, especially after the bank Lehman Brothers declared bankruptcy in September 2008, the starting point of the global financial crisis. \textit{T1.US politics} is considerably larger during the 2008 presidential election campaign.
...
In the example corpus the proportion of the word \textit{economy} in the topics one, three and seven is explored. The \texttt{top.topic.words} of the three chosen topics are \colorbox{yellow}{Inhaltlich checken!}\textit{tax} (T9.economic policy, lightgreen curve), \textit{loss} (T4.economy, orange) and \textit{obama} (T1.US politics, purple).
```{r, fig.width=5.5, fig.height=3.5, fig.align=2}
plotTopicWord(object = corpusFiltered, docs = pagesLDA, ldaresult = result, ldaID = ldaID,
  wordlist = "economy", select = c(9, 4, 1), rel = TRUE, legend = "topleft")
```
The graphic shows that the word \textit{economy} is associated to the topic \colorbox{yellow}{\textit{T9.economic policy, lightgreen curve}} and \colorbox{yellow}{\textit{T4.economy, orange curve}} much more often than with \colorbox{yellow}{\textit{T1.US politics, purple curve}} .
...
plotWordpt(object = corpusFiltered, docs = pagesLDA, ldaresult = result, ldaID = ldaID,
  wordlist = "economy", select = c(9, 4, 1), rel = TRUE)
```
...
plotWordSub(object = corpusFiltered, ldaresult = result, ldaID = ldaID, limit = 1/3,
  select = c(4, 9, 1), search = searcheco, unit = "quarter", legend = "topright")
```
...
The increase of the topic \colorbox{yellow}{\textit{T4.economy} after September 2008} was mentioned before. This should be visible in the following heatmap as well. As compromise between clarity and interpretability \texttt{unit = "quarter"} is chosen.
```{r, fig.height=6, fig.width=10}
plotHeat(object = corpusFiltered, ldaresult = result, ldaID = ldaID, unit = "quarter")
```
\colorbox{yellow}{As expected the \textit{T4.economy}} topic increase is clearly identifiable. The according rectangles are colored more and more red from the first quarter of 2009 in this figure. On the other hand mostly all other quarters of years concerning this topic are colored lightblue. \colorbox{yellow}{Other remarkable quarters} are for example the third and fourth quarter of 2007, where the \colorbox{yellow}{topic \textit{T6.Canadian politics} and topic \textit{T2.stopwords}} have noticeable peaks. The dendrogramm shows that the topics are not very similar to another concerning the absolute deviations of topic proportion from the mean topic proportion per quarter. This supports the findings of clustering the topics with \texttt{clusterTopics}.

## Individual Cases Contemplation - \texttt{topTexts}, \texttt{topicsInText}
Sometimes it is useful to look at some individual cases sometimes. Especially the documents with the highest counts or proportion of words belonging to one topic are of interest. These documents can be extracted by \texttt{topTexts}. By default (\texttt{rel = TRUE}) the proportion is considered. The function requires a \texttt{ldaresult} and the according \texttt{ldaid}. It offers options \texttt{select}, \texttt{limit} and \texttt{minlength}, which control how much articles per topic (default: all topics) are returned (default: \texttt{limit = 20}) and articles of which minimum length (default: \texttt{minlength = 30}) are taken into account. The output value is a matrix of the according IDs.\colorbox{red}{TODO: Hier ist die Sortierung der TopWords im Output wieder anders bei T4 und T9.}

In the example the top four pages from the topics \colorbox{yellow}{\textit{T1.US politics}, {\textit{T4.economy} and \textit{T9.economic policy}} are requested. 
```{r}
topID <- topTexts(ldaresult = result, ldaID = ldaID, select = c(1, 4, 9), limit = 4)
dim(topID)
```
